...
export default function PayrunsPage() {
  ...
  useEffect(() => {
    async function fetchPayruns() {
      try {
        const res = await fetch("/api/v1/payroll/payruns");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold">Payruns</h2>
          <p className="text-sm text-zinc-500">Manage payroll batches and computations.</p>
        </div>
        <button className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-medium flex items-center gap-2">
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-6 py-8 text-center text-zinc-500">No payruns found.</td></tr>
            ) : (
              payruns.map((p) => (
                <tr key={p.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition">
                  ...
                  <td className="px-6 py-4">{p.salaryStructure?.name}</td>
